...
function getUsers() {
    showLoader();
    let collectionRef = db.collection("contractors");
    collectionRef.get()
        .then((querySnapshot) => {
            var documents = [];
    ...
    makeAdmin.forEach(element => {
        element.addEventListener("click", () => {
            ...
            documentRef.update({
                ...
                .then(() => {
                    hideLoader();
                    showAlert('Contractor is now admin');
                    updateTable();
...
function unblockUser(id) {
    ...
    documentRef.update({
        ...
        hideLoader();
        showAlert('Contractor unblocked successfully')
...
function blockUser(id) {
    ...
    documentRef.update({
        ...
        .then(() => {
            hideLoader();
            showAlert('Contractor Blocked successfully');
            updateTable();
...
function authenticateUser(user_username, user_email, pass, busName, busEmail, busPhone, busWebsite, busAddress) {
    ...
    auth.createUserWithEmailAndPassword(user_email, pass)
        .then(userCredential => {
            ...
            const uid = user.uid;
            // console.log(user);
            db.collection("contractors").doc(uid).set({
                userid: uid,
                ...
                .then(() => {
                    ...
                    table
                        ...
                        .draw();
                    getUsers();
                    showAlert('Contractor registered successfully')
                })
                .catch((error) => {
                    hideLoader();
                    showAlert("Error: ", error);
